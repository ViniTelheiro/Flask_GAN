...
class Pix2PixGenerator(nn.Module):
    def __init__(self) -> None: 
        ...
        self.downsamplig = nn.Sequential(
            self.encode(3, 64, 4, apply_batchnorm=False),
            self.encode(64, 128, 4),
            self.encode(128, 256, 4),
            self.encode(256, 512, 4),
            self.encode(512, 512, 4),
            self.encode(512, 512, 4),
            self.encode(512, 512, 4),
            self.encode(512, 512, 4, apply_batchnorm=False)
        )

        self.upsamplig = nn.Sequential(
            self.decode(512*2, 512, 4, apply_dropout=True),
            self.decode(512*2, 512, 4, apply_dropout=True),
            self.decode(512*2, 512, 4, apply_dropout=True),
            self.decode(512*2, 512, 4),
            self.decode(512*2, 256, 4),
            self.decode(256*2, 128, 4),
            self.decode(128*2, 64, 4)
        )
